...
def handle_options():
    if request.method == 'OPTIONS':
        ...
        response.headers['Access-Control-Allow-Origin'] = 'https://chrisostin.github.io'
...
def after_request(response):
    ...
    # Проверяем, есть ли request_id в g
    ...
    if hasattr(g, 'start_time') and isinstance(g.start_time, (int, float)):
        ...
        app_logger.info(f"📤 [{request_id}] {response.status_code} | {round(elapsed*1000,2)}ms")
    else:
        app_logger.info(f"📤 [{request_id}] {response.status_code}")
...
SOLSCAN_URL = "https://public-api.solscan.io"
...
def api_history():
    """Реальная история цен через CoinGecko"""
    timeframe = request.args.get('timeframe', '1d')
    
    # Конвертируем timeframe в дни для CoinGecko
    days_map = {"12h": 1, "1d": 1, "3d": 3, "1m": 30, "3m": 90, "6m": 180, "12m": 365}
    days = days_map.get(timeframe, 1)
    
    try:
        # Получаем историю SOL
        resp = requests.get(
            f"{COINGECKO_URL}/coins/solana/market_chart",
        ...
        if resp.status_code == 200:
            ...
            # Конвертируем SOL цены в MORI
            result = []
            for ts, price in prices:
                mori_price = price * 0.00004
                result.append({
                    "x": datetime.fromtimestamp(ts/1000).isoformat(),
...
def api_whales():
    """Реальные киты через Solscan"""
    try:
        # Реальный адрес токена MORI
        token_address = "8ZHE4ow1a2jjxuoMfyExuNamQNALv5ekZhsBn5nMDf5e"
        resp = requests.get(
            f"{SOLSCAN_URL}/token/holders",
        ...
        if resp.status_code == 200:
            ...
            result = []
            total_supply = 1000000000  # 1B MORI
            for h in holders[:5]:  # Берём топ-5
                amount = h.get("amount", 0) / 1e9  # если в лампортах
                result.append({
                    "address": h.get("owner", "unknown")[:10] + "...",
                    "amount": int(amount),
                    "percentage": round(amount / total_supply * 100, 2)
                })
            return jsonify(result)
    except Exception as e:
        print(f"Ошибка получения китов: {e}")
    
    # Фолбэк
    return jsonify([
        {"address": "7xKXtg2CW87d97TXJSDpbD5jBkquiTzureRbJZg6yNQY", "amount": 15234567, "percentage": 1.52},
        {"address": "Dx2R5xKp9LmN7QwE3tY1uI8oP0zXcVbN", "amount": 12345678, "percentage": 1.23},
        {"address": "Fh3T6yLp0QwR2nM9kZ7xCvB1aAsDfG", "amount": 8765432, "percentage": 0.88},
        {"address": "Gj2R5xKp9LmN7QwE3tY1uI8oP0zXcVbN", "amount": 5432109, "percentage": 0.54},
        {"address": "Hk4U7iOp1RwE3yT5uI8oP0zXcVbNm", "amount": 3210987, "percentage": 0.32}
    ])
# ========== РЕГИСТРАЦИЯ РОУТОВ ==========
